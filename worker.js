...
// In-memory Auth & History store for Worker instance
const workerUsersByEmail = new Map();
const workerUsersById = new Map();
const workerUserTokens = new Map();
const workerUserHistories = new Map();

async function hashPasswordWorker(password) {
  const encoder = new TextEncoder();
  const data = encoder.encode(password + '_aramshkon_salt_v1');
  const hashBuffer = await crypto.subtle.digest('SHA-256', data);
  const hashArray = Array.from(new Uint8Array(hashBuffer));
  return hashArray.map((b) => b.toString(16).padStart(2, '0')).join('');
}

async function findUserByEmailWorker(email, env) {
  const cleanEmail = (email || '').toLowerCase().trim();
  if (workerUsersByEmail.has(cleanEmail)) {
    return workerUsersByEmail.get(cleanEmail);
  }
  const kv = env?.AUTH_KV || env?.COUPLE_KV || env?.ARAMKON_KV || env?.KV;
  if (kv) {
    try {
      const raw = await kv.get('user:email:' + cleanEmail);
      if (raw) {
        const user = JSON.parse(raw);
        workerUsersByEmail.set(cleanEmail, user);
        workerUsersById.set(user.id, user);
        return user;
      }
    } catch {}
  }
...
async function findUserByIdWorker(id, env) {
  if (workerUsersById.has(id)) {
    return workerUsersById.get(id);
  }
  const kv = env?.AUTH_KV || env?.COUPLE_KV || env?.ARAMKON_KV || env?.KV;
  if (kv) {
    try {
      ...
      if (raw) {
        const user = JSON.parse(raw);
        workerUsersById.set(id, user);
        workerUsersByEmail.set(user.email.toLowerCase(), user);
        return user;
      }
    } catch {}
  }
...
async function saveUserWorker(user, env) {
  workerUsersByEmail.set(user.email.toLowerCase(), user);
  workerUsersById.set(user.id, user);
  const kv = env?.AUTH_KV || env?.COUPLE_KV || env?.ARAMKON_KV || env?.KV;
  if (kv) {
    try {
      await Promise.all([
        kv.put('user:email:' + user.email.toLowerCase(), JSON.stringify(user)),
...
async function createTokenWorker(user, env) {
  ...
  if (kv) {
    try {
      await kv.put('token:' + token, user.id, { expirationTtl: 86400 * 30 });
    } catch {}
  }
  return token;
}

async function getUserByTokenWorker(req, env) {
  const authHeader = req.headers.get('Authorization') || req.headers.get('authorization') || '';
  if (!authHeader.startsWith('Bearer ')) return null;
  const token = authHeader.replace(/^Bearer\s+/i, '').trim();
  if (!token) return null;

  let userId = workerUserTokens.get(token);
  if (!userId) {
    const kv = env?.AUTH_KV || env?.COUPLE_KV || env?.ARAMKON_KV || env?.KV;
    if (kv) {
      try {
        userId = await kv.get('token:' + token);
        if (userId) workerUserTokens.set(token, userId);
      } catch {}
    }
  }
...
async function getUserHistoryWorker(userId, env) {
  if (workerUserHistories.has(userId)) {
    return workerUserHistories.get(userId) || [];
  }
  const kv = env?.AUTH_KV || env?.COUPLE_KV || env?.ARAMKON_KV || env?.KV;
  if (kv) {
    try {
      const raw = await kv.get('history:' + userId);
      if (raw) {
        const items = JSON.parse(raw);
        workerUserHistories.set(userId, items);
        return items;
      }
    } catch {}
  }
...
async function saveUserHistoryWorker(userId, historyItems, env) {
  workerUserHistories.set(userId, historyItems);
  const kv = env?.AUTH_KV || env?.COUPLE_KV || env?.ARAMKON_KV || env?.KV;
  if (kv) {
    try {
      await kv.put('history:' + userId, JSON.stringify(historyItems));
    } catch {}
  }
}

async function getUserStatsWorker(userId, env) {
  const history = await getUserHistoryWorker(userId, env);
  let coupleSessionsCount = 0;
  for (const session of workerCoupleSessions.values()) {
    if (session.participantA?.userId === userId || session.participantB?.userId === userId) {
      coupleSessionsCount++;
    }
  }
  return {
    ...
    coupleSessionsCount,
    savedResponsesCount: history.filter((h) => h.analysis?.suggestedResponses).length,
  };
}
...
export default {
  async fetch(request, env) {
    ...
    if (request.method === 'GET' && (normalizedPath === '/api/auth/me' || normalizedPath === '/auth/me')) {
      const user = await getUserByTokenWorker(request, env);
      if (!user) {
        return new Response(
          JSON.stringify({ error: 'UNAUTHORIZED', message: 'نشست کاربری نامعتبر است.' }),
          { status: 401, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
      const stats = await getUserStatsWorker(user.id, env);
      return new Response(
        JSON.stringify({
          success: true,
    ...
    if (request.method === 'GET' && (normalizedPath === '/api/history' || normalizedPath === '/history')) {
      const user = await getUserByTokenWorker(request, env);
      if (!user) {
        return new Response(
          JSON.stringify({ error: 'UNAUTHORIZED', message: 'برای مشاهده تاریخچه وارد شوید.' }),
          { status: 401, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
      const history = await getUserHistoryWorker(user.id, env);
      return new Response(
    ...
    if ((request.method === 'POST' || request.method === 'PUT') && (normalizedPath === '/api/auth/profile' || normalizedPath === '/auth/profile' || normalizedPath === '/api/auth/update' || normalizedPath === '/auth/update')) {
      ...
      if (!name || typeof name !== 'string' || name.trim().length < 2) {
        return new Response(
          JSON.stringify({ error: 'INVALID_NAME', message: 'لطفاً نام معتبری وارد کنید.' }),
          { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
      user.name = name.trim();
      await saveUserWorker(user, env);
      return new Response(
        JSON.stringify({
          success: true,
          user: { id: user.id, email: user.email, name: user.name, createdAt: user.createdAt },
    ...
    if (request.method === 'DELETE' && (normalizedPath === '/api/auth/account' || normalizedPath === '/auth/account' || normalizedPath === '/api/auth/delete' || normalizedPath === '/auth/delete')) {
      ...
      workerUsersByEmail.delete(user.email.toLowerCase());
      workerUsersById.delete(user.id);
      workerUserHistories.delete(user.id);
      const kv = env?.AUTH_KV || env?.COUPLE_KV || env?.ARAMKON_KV || env?.KV;
      if (kv) {
        try {
          await Promise.all([
            kv.delete('user:email:' + user.email.toLowerCase()),
            kv.delete('user:id:' + user.id),
            kv.delete('history:' + user.id),
          ]);
        } catch {}
      }
      return new Response(
        JSON.stringify({ success: true, message: 'حساب کاربری شما با موفقیت حذف شد.' }),
        { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    // History: DELETE /api/history/clear
    if (request.method === 'DELETE' && (normalizedPath === '/api/history/clear' || normalizedPath === '/history/clear')) {
      const user = await getUserByTokenWorker(request, env);
      if (!user) {
        return new Response(
          JSON.stringify({ error: 'UNAUTHORIZED', message: 'نشست کاربری نامعتبر است.' }),
      ...
      await saveUserHistoryWorker(user.id, [], env);
      return new Response(
        JSON.stringify({ success: true, message: 'تاریخچه تحلیل‌ها پاک شد.' }),
        { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    // History: DELETE /api/history/:id
    if (request.method === 'DELETE' && (normalizedPath.startsWith('/api/history/') || normalizedPath.startsWith('/history/'))) {
      const user = await getUserByTokenWorker(request, env);
      if (!user) {
        return new Response(
          JSON.stringify({ error: 'UNAUTHORIZED', message: 'نشست کاربری نامعتبر است.' }),
      ...
      const history = await getUserHistoryWorker(user.id, env);
      const updated = history.filter((item) => item.id !== recordId && item.analysis?.id !== recordId);
      await saveUserHistoryWorker(user.id, updated, env);
      return new Response(
        JSON.stringify({ success: true, history: updated }),
        { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    if (request.method === 'POST') {
      try {
        ...
        if (normalizedPath === '/api/auth/register' || normalizedPath === '/auth/register') {
          ...
          if (!cleanEmail || !cleanEmail.includes('@') || !cleanEmail.includes('.')) {
            return new Response(
              JSON.stringify({ error: 'INVALID_EMAIL', message: 'لطفاً یک ایمیل معتبر وارد کنید.' }),
              { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
            );
          }
          ...
          if (!password || typeof password !== 'string' || password.length < 4) {
            return new Response(
              JSON.stringify({ error: 'INVALID_PASSWORD', message: 'رمز عبور باید حداقل ۴ کاراکتر باشد.' }),
              { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
            );
          }
          ...
          if (!cleanName || cleanName.length < 2) {
            return new Response(
              JSON.stringify({ error: 'INVALID_NAME', message: 'لطفاً نام خود را وارد کنید.' }),
              { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
            );
          }

          const existing = await findUserByEmailWorker(cleanEmail, env);
          if (existing) {
            return new Response(
              JSON.stringify({ error: 'USER_EXISTS', message: 'این ایمیل قبلاً ثبت‌نام کرده است. لطفاً وارد شوید.' }),
              { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
            );
          }
          ...
          await saveUserWorker(user, env);
          const token = await createTokenWorker(user, env);
          const stats = await getUserStatsWorker(user.id, env);
        ...
        if (normalizedPath === '/api/auth/login' || normalizedPath === '/auth/login') {
          const { email, password } = body;
          const cleanEmail = (typeof email === 'string' && email.trim().toLowerCase()) || '';

          if (!cleanEmail || !password) {
            return new Response(
              JSON.stringify({ error: 'INVALID_INPUT', message: 'لطفاً ایمیل و رمز عبور را وارد کنید.' }),
              { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
            );
          }

          const user = await findUserByEmailWorker(cleanEmail, env);
          if (!user) {
            return new Response(
              JSON.stringify({ error: 'INVALID_CREDENTIALS', message: 'ایمیل یا رمز عبور اشتباه است.' }),
              { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
            );
          }
        ...
        // History: POST /api/history/sync
        if (normalizedPath === '/api/history/sync' || normalizedPath === '/history/sync') {
          const user = await getUserByTokenWorker(request, env);
          if (!user) {
            return new Response(
              JSON.stringify({ error: 'UNAUTHORIZED', message: 'نشست کاربری نامعتبر است.' }),
          ...
          const existingHistory = await getUserHistoryWorker(user.id, env);
          const existingIds = new Set(existingHistory.map((h) => h.id));
          const newItems = localItems.filter((item) => item && item.id && !existingIds.has(item.id));
          const mergedHistory = [...newItems, ...existingHistory];
          await saveUserHistoryWorker(user.id, mergedHistory, env);
          return new Response(
            JSON.stringify({ success: true, history: mergedHistory }),
            { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
          );
        }

        // History: POST /api/history/save
        if (normalizedPath === '/api/history/save' || normalizedPath === '/history/save') {
          const user = await getUserByTokenWorker(request, env);
          if (!user) {
            return new Response(
              JSON.stringify({ error: 'UNAUTHORIZED', message: 'نشست کاربری نامعتبر است.' }),
          ...
          if (!record || !record.id) {
            return new Response(
              JSON.stringify({ error: 'INVALID_RECORD', message: 'رکورد وارد شده معتبر نیست.' }),
              { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
            );
          }
          const history = await getUserHistoryWorker(user.id, env);
          const updatedHistory = [record, ...history.filter((h) => h.id !== record.id)];
          await saveUserHistoryWorker(user.id, updatedHistory, env);
          return new Response(
            JSON.stringify({ success: true, record, history: updatedHistory }),
            { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
          );
        }
...
فقط و فقط یک JSON معتبر مطابق ساختار زیر بازگردان:
{
  "replies": {
    ...
    "friendly": "پیام دوستانه"
        ...
        // ---------------------------------------------------------------------------------
        if (normalizedPath === '/api/analyze' || normalizedPath === '/analyze' || normalizedPath === '/') {
